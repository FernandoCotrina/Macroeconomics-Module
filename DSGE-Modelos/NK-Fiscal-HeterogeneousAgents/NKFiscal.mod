%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                        NK Model with Government                         %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% "addpath 'C:\dynare\6.2\matlab'" // To start Dynare
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
gammaR, gammaPI, gammaY, tau_css, tau_lss, tau_kss, phic, omegaR, 
Psi1, Psi2, chi;
...
*alpha1*CMss)/(Pss*Rss*(1+tau_css)))-(delta*alpha1*CMss/Rss)-(phiB/Pss)*((1/RBss)-1)+phi_TRANS);
...
KP = (1-delta)*KP(-1) + delta*IP;
    ...
    % Euler’s equation (Public bond)
LAMBDAR + RB= LAMBDAR(+1);

    % Non-Ricardian household Lagrangian
LAMBDANR + P + (tau_css/(1+tau_css))*tau_c = (sigma/((1-phic)*(1-phic*beta)))*(phic*beta*(CNR(+1)-CNR)-(CNR-CNR(-1)));

    % Phillips equation for non-Ricardian household wages
...
Y = A + alpha1*(U+KP(-1)) + alpha2*L + alpha3*KG(-1);
    ...
    % Problem of the firm trade-off (MRS=Relative price)
L- U- KP(-1) = R- W;
...
SIG = rhoIG*SIG(-1) + e_IG;
    ...
    % Shock in Transfer of Income
STRANS = rhoTRANS*STRANS(-1) + e_TRANS;

    % Shock in tax on Consumption
Stau_c = rhotau_c*Stau_c(-1)- e_tau_c;

    % Shock in tax on Labor Income
Stau_l = rhotau_l*Stau_l(-1)- e_tau_l;

    % Shock in tax on Capital Income
